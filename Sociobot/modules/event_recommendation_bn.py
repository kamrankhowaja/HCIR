...
def recommended_event_probs(interest, score):
    ...
    boost_map = {"High": 8.0, "Medium": 4.5, "Low": -0.6}
...
class EventRecommendationNetwork:
    """Object-oriented interface for building and querying the event BN."""

    def __init__(self, states: Optional[dict] = None, arcs: Optional[list] = None):
        self.states = states or STATES
        self.arcs = arcs or BN_ARCS
        self.bn = None

    def build(self):
        """Build and return the Bayesian Network with the existing CPT logic."""
        bn = gum.BayesNet("SocialEventRecommendationBN")

        for node_name, labels in self.states.items():
        ...
        for mood, env, grp in itertools.product(
            self.states["Mood"], self.states["EnvironmentPreference"], self.states["GroupSize"]
        ):
            bn.cpt("SocialEnergy")[
        ...
        for env, grp in itertools.product(
            self.states["EnvironmentPreference"], self.states["GroupSize"]
        ):
        ...
        for se, tr, grp, bud in itertools.product(
            self.states["SocialEnergy"], self.states["TimeRestrictions"],
            self.states["GroupSize"],    self.states["BudgetPreference"]
        ):
            bn.cpt("InterestType")[
        ...
        for hist, it in itertools.product(
            self.states["UserPreferenceHistory"], self.states["InterestType"]
        ):
        ...
        for it, score in itertools.product(
            self.states["InterestType"], self.states["EventScoreMatching"]
        ):
    ...
    def validate_evidence(self, evidence: dict) -> None:
        for node, value in evidence.items():
            ...
            if value not in self.states[node]:
                raise ValueError(
                    f"Invalid value '{value}' for node '{node}'. "
                    f"Allowed: {self.states[node]}"
                )

    def get_posterior_dict(self, inference, node_name: str) -> dict:
        posterior = inference.posterior(node_name)
        return {
            state: float(posterior[{node_name: state}])
            for state in self.states[node_name]
        }

    def recommend(self, evidence: dict) -> tuple[list, dict]:
        """
        ...
        ranking = sorted(
            [(ev, float(posterior[{"RecommendedEvent": ev}]))
             for ev in self.states["RecommendedEvent"]],
            key=lambda x: x[1], reverse=True
        )

        inferred_nodes = {
            "SocialEnergy":      self.get_posterior_dict(inference, "SocialEnergy"),
            "TimeRestrictions":  self.get_posterior_dict(inference, "TimeRestrictions"),
            "InterestType":      self.get_posterior_dict(inference, "InterestType"),
            "EventScoreMatching":self.get_posterior_dict(inference, "EventScoreMatching"),
        }
    ...
    def save_demo_outputs(self, output_dir=DEMO_OUTPUT_DIR):
        ...
        self.save_bn(bn, bn_file)
        self.export_bn_to_dot(bn, dot_file)
        self.render_dot_to_png(dot_file, png_file)
...
def build_event_recommendation_bn():
    return EventRecommendationNetwork().build()


#  INFERENCE API
def validate_evidence(evidence: dict) -> None:
    EventRecommendationNetwork().validate_evidence(evidence)


def get_posterior_dict(inference, node_name: str) -> dict:
    return EventRecommendationNetwork().get_posterior_dict(inference, node_name)
...
def recommend_events(evidence: dict) -> tuple[list, dict]:
    ...
    if _default_network is None:
        _default_network = EventRecommendationNetwork()
    return _default_network.recommend(evidence)


#  EXPORT HELPERS
def save_bn(bn, filename):
    EventRecommendationNetwork().save_bn(bn, filename)


def export_bn_to_dot(bn, dot_filename):
    EventRecommendationNetwork().export_bn_to_dot(bn, dot_filename)


def render_dot_to_png(dot_filename, png_filename):
    return EventRecommendationNetwork().render_dot_to_png(dot_filename, png_filename)


def save_demo_bn_outputs(bn, output_dir=DEMO_OUTPUT_DIR):
    network = EventRecommendationNetwork()
    network.bn = bn
    return network.save_demo_outputs(output_dir)
